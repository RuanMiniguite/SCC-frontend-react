...
const UpdateComissao = () => {
    ...
    useEffect(() => {
        axios.get(`${API}/taxacomissao/${params.codComissao}`)
            .then(res => {
                ...
                setValorLimite(res.data.valorLimite)
                setAnoCasa(res.data.anoCasa)
    ...
    const handleSubmit = async (e) => {
        ...
        await axios.put(`${API}/taxacomissao/${params.codComissao}`, {
            codTaxaComissao: params.codComissao, taxa, valorLimite, anoCasa: parseInt(anoCasa), cargo
        })
            .then(res => {
                if (res.status === 200) {
                    alert("Comissão: " + res.data.codTaxaComissao + ' - ' + res.data.cargo + " atualizada com sucesso!")
                    navigateToComissoes()
                } else {
